...
def initmatrix(num_of_teams, num_of_alliances, json_file, teams, OPR):
...
def OPR_Calc(initmatrixresults): 
...
def main():
	...
	OPR_results = OPR_Calc(OPR_matrix)
	display(OPR_results, teams)
	

def display(OPR_results, teams):
	print
	count = 0
	for result in OPR_results:
		OPR = float(result)
		print (teams[count], "				", "OPR: ", float(result))
		count += 1
