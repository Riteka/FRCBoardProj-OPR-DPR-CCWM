...
def display(OPR_Results, DPR_Results, teams):
	...
	count = 0
	print ('Team Number    ', 'OPR               ', 'DPR             ', 'CCWM    ')
	for OPR in OPR_Results:
		OPR = float(OPR[0])
		DPR = float(DPR_Results[count])
		CCWM = float(CCWM_Calc(OPR, DPR_Results[count]))
...
def ask_user_inputs():
	...
	import argparse
	parser = argparse.ArgumentParser(description="Access .csv file containing match data for desired event or Blue Alliance API match data for the desired event in order to find OPR, DPR, CCWM. The input should be either the name of a .csv file located in the same directory as this program or event key.")
	parser.add_argument('--csv_filename', default='', type=str, help="Input .csv filename (example: 2019copy.csv) (default:'')")
	parser.add_argument('--event_key', default='', type=str, help="Input event key (example: 2019abca) (default: '')")
	args = parser.parse_args()
	return ([args.csv_filename, args.event_key])
